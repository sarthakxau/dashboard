...
const failedColumns: Column<DbTransaction>[] = [
  ...
  { key: 'error', header: 'Error', render: (r) => <span className="text-xs text-rose-600 max-w-xs truncate block">{r.error_message ?? '—'}</span> },
...
export default function Health() {
  ...
  const priceStatus = priceAge < PRICE_STALE_THRESHOLD_MS ? 'healthy' : priceAge < 15 * 60 * 1000 ? 'warning' : 'error';
    ...
    <>
      ...
      <div className="p-6 space-y-6 max-w-7xl">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-primary mb-4">Price Oracle</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <p className="text-xs text-secondary mb-1">Gold Price (USD)</p>
              <p className="font-mono">{o?.currentPriceUSD ? formatUSD(o.currentPriceUSD) : '—'}</p>
            </div>
            <div>
              <p className="text-xs text-secondary mb-1">Gold Price (INR)</p>
              <p className="font-mono">{o?.currentPriceINR ? formatINR(o.currentPriceINR) : '—'}</p>
            </div>
            <div>
              <p className="text-xs text-secondary mb-1">24h Range (USD)</p>
            ...
            <div>
              <p className="text-xs text-secondary mb-1">Price Gaps (7d)</p>
              <p className="font-mono">{o?.priceGaps7d ?? '—'} gaps (&gt;10 min)</p>
            </div>
          </div>
        </div>
        ...
        {/* Failed Transactions */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-primary mb-4">Failed Transactions</h3>
          <div className="flex gap-6 mb-4 text-sm">
            <div>
              <p className="text-xs text-secondary">Last 24h</p>
              <p className="text-lg font-semibold font-mono text-rose-600">{f?.count24h ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-secondary">Last 7d</p>
              <p className="text-lg font-semibold font-mono text-rose-600">{f?.count7d ?? '—'}</p>
            </div>
          </div>

          {f?.errorGroups && f.errorGroups.length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-secondary mb-2">Error Groups</p>
              <div className="space-y-1">
                {f.errorGroups.slice(0, 5).map((eg) => (
                  <div key={eg.error} className="flex justify-between text-xs">
                    <span className="text-rose-600 truncate max-w-md">{eg.error}</span>
                    <span className="text-secondary font-mono">{eg.count}x</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Stale Data */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-primary mb-4">Stale Data Alerts</h3>
          <div className="flex gap-6 text-sm">
            <div className="flex items-center gap-2">
              <StatusDot status={s && s.stalePendingTxs > 0 ? 'warning' : 'healthy'} />
              <span>{s?.stalePendingTxs ?? 0} pending txs older than 10 min</span>
            </div>
            <div className="flex items-center gap-2">
              <StatusDot status={s && s.giftsExpiringIn48h > 0 ? 'warning' : 'healthy'} />
              <span>{s?.giftsExpiringIn48h ?? 0} gifts expiring within 48h</span>
            </div>
          </div>
        </div>
        ...
        {/* DB Stats */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-primary mb-3">Database Stats</h3>
